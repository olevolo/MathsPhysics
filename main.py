...
class Application:
    ...
    def _take_x(self, element):
        return self.vertices[element][0]

    def _take_y(self, element):
        return self.vertices[element][1]
    ...
    def triangulate(self):
        ...
        self.vertices = raw_info['vertices']

        boundary_points_dict = {
            tuple(segment): self._boundary_segments(segment, raw_info['vertices']) for segment in self.segments
        }
        ...
        for segment, points in boundary_points_dict.items():
            x1, x2 = raw_info['vertices'][segment[0]][0], raw_info['vertices'][segment[1]][0]
            y1, y2 = raw_info['vertices'][segment[0]][1], raw_info['vertices'][segment[1]][1]
        ...
        for segment, points in boundary_points_dict.items():
            boundary_points_dict[segment] = [[boundary_points_dict[segment][i], boundary_points_dict[segment][(i+1)]]for i in range(len(points)-1)]
